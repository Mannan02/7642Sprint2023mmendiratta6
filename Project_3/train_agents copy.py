...
def main(n_cpus, n_gpus, env_name,
    ...
    policies['default_policy'] = (None, obs_space[f'player_0'], act_space[f'player_0'], {})
    ...
    def policy_mapping_fn_impala2(agent_id, episode, **kwargs):
        if agent_id == 'default_policy':
            return 'default_policy'
        else:
            return f"agent_{int(agent_id.split('_')[1])}"
            
        
    policy_mapping_fn_impala = lambda agent_id, episode, **kwargs: \
        'default_policy' if agent_id == 'default_policy' else 'agent_0'
        # 'default_policy' if agent_id == 'default_policy' else f"agent_{int(agent_id.split('_')[1])}"

    default_multiagent = {
        'policies': policies,
        'policy_mapping_fn': policy_mapping_fn_impala2,
    }

    shared_policy = {'agent_0': gen_policy(0)}
    shared_policy['default_policy'] = (None, obs_space[f'player_0'], act_space[f'player_0'], {})
    shared_policy_mapping_fn = lambda agent_id, episode, **kwargs: 'agent_0'
    shared_policy_mapping_fn_impala = lambda agent_id, episode, **kwargs: \
        'default_policy' if agent_id == 'default_policy' else 'agent_0'
    shared_multiagent = {
        'policies': shared_policy,
        'policy_mapping_fn': shared_policy_mapping_fn_impala,
    }
    ModelCatalog.register_custom_model("my_torch_model", TorchRNNModel)
    config = {
        'env': env_name,
        'framework': 'torch',
